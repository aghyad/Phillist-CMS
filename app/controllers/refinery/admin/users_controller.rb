  ...
  module Admin
    class UsersController < Refinery::AdminController
      ...
      def update
        ...
        if current_refinery_user.id == @user.id and ((!params[:user][:plugins].nil? and params[:user][:plugins].exclude?("refinery_users")) ) #|| @selected_role_names.map(&:downcase).exclude?("refinery"))
...
Rails.logger.debug "\n\n\n====\n#{@available_roles.inspect}\n--------\n#{current_refinery_user.inspect}\n--------\n#{RolesUsers.where(:user_id=>current_refinery_user.id).inspect}\n====\n\n\n"
